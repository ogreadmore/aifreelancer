...
if (contactForm) {
      ...
      applyPrefill(
        {
          ...
          contextMessage: link.dataset.prefillLabel
            ? `${link.dataset.prefillLabel} has been selected for you below.`
            : "Your selection is prepared in the form below.",
        },
        true,
      );
    });
  ...
  contactForm.addEventListener("submit", (event) => {
    ...
    const subjectParts = ["Mobile Cryo Pro — New consultation inquiry"];

    if (values.service) {
      subjectParts.push(`— ${values.service}`);
    }

    if (values.name) {
      subjectParts.push(`(${values.name})`);
    }
